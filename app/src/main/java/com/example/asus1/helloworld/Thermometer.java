...
public class Thermometer extends SurfaceView implements SurfaceHolder.Callback, Runnable{
    ...
    public void run() {
        //不断进行绘制
        int minTemp = 32, maxTemp = 40;
        Random random = new Random();
        float current = 36, last = 36;

        while(isRunning){
            while(Math.abs(current - last) > 1.0) current += (last - current) / 2.0;
            setTargetTemperature(current);
            last = current;
            current = random.nextInt(maxTemp)%(maxTemp-minTemp+1) + minTemp;
